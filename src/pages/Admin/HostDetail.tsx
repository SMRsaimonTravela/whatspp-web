  ...
  const loadConversation = useCallback(async (prepend = false) => {
    ...
    try {
      const res = await adminService.getHostGuestMessages(
        selectedGuest!.id,
        { page: conversationPage, limit: 50 }
      );
      if (res.success) {
        setConversation(prev => prepend ? [...res.data.reverse(), ...prev] : res.data.reverse());
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white">
          ...
          >
            <div className="px-4 md:px-6 pb-3 md:pb-4">
              <div className="flex items-center gap-2 flex-wrap mb-3">
                <span className={`px-2 md:px-3 py-1 rounded-full text-xs font-semibold ${host.aiAutoReplyEnabled
                ...
                <span className={`px-2 md:px-3 py-1 rounded-full text-xs font-semibold ${host.status === 'approved'
                  ...
                  {host.commissionRule ? (
                    <>
                      ...
                      <div className="flex items-center gap-2 mt-2 flex-wrap">
                        <span className={`px-2 py-0.5 rounded text-xs font-semibold ${host.commissionRule.status === 'active'
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Left: Guest List */}
        <div className={`w-full md:w-1/3 border-r border-gray-200 dark:border-gray-700 flex flex-col bg-white dark:bg-gray-800 ${showMobileView === 'guests' ? 'block' : 'hidden md:block'
          }`}>
                  ...
                  <div
                    ...
                    onClick={() => {
                      if (selectedGuest?.id !== g.id) {
                        ...
                        setConversationPage(1);
                        shouldScrollToBottom.current = true;
                        // Switch to conversation view on mobile
                        setShowMobileView('conversation');
                      }
                    }}
                    className={`p-3 cursor-pointer border-b border-gray-100 dark:border-gray-700/50 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors ${selectedGuest?.id === g.id ? 'bg-gray-100 dark:bg-gray-700' : 'bg-white dark:bg-gray-800'
                      }`}
                  >
        ...
        {/* Right: Conversation */}
        <div className={`flex-1 flex flex-col bg-[#efeae2] dark:bg-gray-900 ${showMobileView === 'conversation' ? 'block' : 'hidden md:flex'
          }`}>
          {/* Chat Header */}
          <div className="p-4 border-b border-gray-200 dark:border-gray-700 bg-[#f0f2f5] dark:bg-gray-800 shadow-sm">
            <div className="flex items-center gap-3">
              ...
              <button
                onClick={() => setShowMobileView('guests')}
          ...
          <div
            ...
            style={{
              scrollbarWidth: 'thin',
              scrollbarColor: '#9ca3af transparent',
              backgroundImage: 'linear-gradient(rgba(0, 0, 0, 0.50), rgba(0, 0, 0, 0.15)), url(/whatsappchatbg.png)',
              backgroundSize: 'auto',
          ...
          >
            ...
            {loading.conversation && conversation.length === 0 ? (
